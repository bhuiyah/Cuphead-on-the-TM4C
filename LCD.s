; LCD.s
; Student names: Sajid Bhuiyan and Calvin Heischman
; Last modification date: 3/26/22
...
loop1
		LDR R4, =SSI0_SR_R
		LDR R5, [R4]
		AND R5, #0x10 ;isolates bit 4
		CMP R5, #0
		BNE loop1
		
		LDR R4, =GPIO_PORTA_DATA_R ;Set PA6 to zero to send a command      D = 1, C = 0
		LDR R5,[R4]
		BIC R5, #0x40
		STR R5,[R4]
    
		LDR R4, =SSI0_DR_R ;Store command(R0) into SSIO_DR_R
		STRB R0,[R4]
		
loop2
		LDR R4, =SSI0_SR_R ;Waits for SSIO_SR_R bit 4 to be cleared
		LDR R5, [R4]
		AND R5, #0x10 ;isolates bit 4
		CMP R5, #0
		BNE loop2
